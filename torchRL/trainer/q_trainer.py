        ...
        if (
            ...
            print(
                f"Last {self.cfg.TRAIN.NUM_EPISIODES_AVG_STEPS} avg steps exceeded \
                    {self.cfg.TRAIN.AVG_STEPS_TO_TERMINATE} times. Quit training."
            )
            return True
        else:
            return False
...
class SARSATrainer(QTrainer):
    ...
    def run_single_episode(self, episode_num):
        ...
        return (state, next_state, reward, action, done, steps)

    def train(self):
        """Train the q network"""
        for episode_num in range(self.cfg.TRAIN.NUM_EPISODES):
            state, next_state, reward, action, done, steps = self.run_single_episode(episode_num)
            self.steps_history.append(steps)

            # update the q_net (experience replay)
            self.update(state, next_state, reward, action, done, steps)
    ...
    def update(self, state, next_state, reward, action, done, steps):
